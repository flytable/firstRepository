...
error_msg() {
    local msg="$1"
    test -z "$msg" && msg="Unknown error"					#test命令测试$1参数是否为空，为空时，msg赋值为unknown error
    echo "ERROR: $msg" >&2									#stderr的文件描述符为2，Unix程序会向stderr流中写入错误信息
    exit 1
}
...
install_deps() {																		#安装工具包
    pkgs="$1"
    [ -z "${pkgs}" ] && error_msg "Usage: install_deps pkgs"							#判断第一个参数是否为空，为空报错误信息
    # skip_install parmater is optional.
    skip_install="$2"																	

    if [ "${skip_install}" = "True" ] || [ "${skip_install}" = "true" ]; then			#如果第二个参数为true，那么跳过安装步骤
